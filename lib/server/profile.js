'use strict';

var find_options = require('./query');

function storage (collection, ctx) {
   var ObjectID = require('mongodb').ObjectID;

  function create (obj, fn) {
    obj.created_at = (new Date( )).toISOString( );
    api().insert(obj, function (err, doc) {
      fn(null, doc);
    });
    ctx.bus.emit('data-received');
  }

  function save (obj, fn) {
    obj._id = new ObjectID(obj._id);
    if (!obj.created_at) {
      obj.created_at = (new Date( )).toISOString( );
    }
    api().save(obj, function (err) {
      //id should be added for new docs
      fn(err, obj);
    });
    ctx.bus.emit('data-received');
  }

  function list(fn) {
      return api().find({}).sort({ startDate: -1 }).toArray(fn);
  }

  function list_query(opts, fn) {

      storage.queryOpts = {
        walker: {
          mills: parseInt
        },
        walkerSort: {
          mills: parseInt,
          startDate: parseInt
        }
          , dateField: 'startDate'
      };

      function limit() {
          if (opts && opts.count) {
              return this.limit(parseInt(opts.count));
          }
          return this;
      }

      return limit.call(api()
        .find(query_for(opts))
        .sort(opts && opts.sort && query_sort(opts) || { startDate: 1 }), opts)
        .toArray(fn);
  }


  function query_for(opts) {
    console.log('Opts before', opts.sort);
      var retVal = find_options(opts, storage.queryOpts);
    console.log('Opts after', opts.sort);
      console.warn(retVal);
      return retVal;
  }

  function query_sort(opts) {
    console.log('Opts sort', opts.sort);
      var retVal = find_options.walkerSort(opts, storage.queryOpts);
    console.log('Opts sort after', opts.sort);
      console.warn(retVal);
      return retVal;
  }
  function last (fn) {
    return api().find().sort({startDate: -1}).limit(1).toArray(fn);
  }

  function remove (_id, fn) {
    var objId = new ObjectID(_id);
    api( ).remove({ '_id': objId }, fn);

    ctx.bus.emit('data-received');
  }

  function api () {
    return ctx.store.collection(collection);
  }
  
  api.list = list;
  api.list_query = list_query;
  api.create = create;
  api.query_for = query_for;
  api.save = save;
  api.remove = remove;
  api.last = last;
  api.indexedFields = ['startDate'];
  return api;
}

module.exports = storage;
