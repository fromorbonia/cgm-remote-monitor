...
daytoday.report = function report_daytoday(datastorage,sorteddaystoshow,options) {
  ...
  d3.select('body').append('svg')
    ...
    console.log("Treatments: ", datastorage.treatments);
    console.log("Data Storage Profiles: ", datastorage.profiles);
    ...
    var pdataActive = profile.data[0];
	    for (let pdata in profile.data) {
            console.log("PData Mills: ", profile.data[pdata].mills);
            if (Number(tstTimeAfterMidnight) >= Number(profile.data[pdata].mills))
		    {
                pdataActive = profile.data[pdata];
			    break;
		    }
    }
