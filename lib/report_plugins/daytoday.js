...
daytoday.report = function report_daytoday (datastorage, sorteddaystoshow, options) {
  ...
  function drawChart (day, data, options) {
    ...
    console.log("Day: ", day, "TZ: ", moment.tz.guess());
    ...
    console.log("DS.Profiles: ", datastorage.profiles);
    ...
    var tstTimeAfterMidnight = moment(from);
    tstTimeAfterMidnight.add(10, 'minutes');
    console.log("Test Time: ", tstTimeAfterMidnight);
    console.log("Test Time Mills: ", Number(tstTimeAfterMidnight));
    var tstActiveTreatment = profile.getCurrentProfile(tstTimeAfterMidnight);
    
    var pdataActive = profile.data[0];
        for (var i = 0; i < profile.data.length; i++) {
            console.log("PData Mills: ", profile.data[i].mills);
            if (Number(tstTimeAfterMidnight) >= Number(profile.data[i].mills))
            {
                pdataActive = profile.data[i];
                break;
            }
    }

    console.log("PData Active: ", pdataActive.startDate);
    console.log("Test Active Treat: ", tstActiveTreatment);
    var tstGTB = profile.getTempBasal(date);
    console.log("Test GTB: ", tstGTB);
    ...
    for (var dt = moment(from); dt < to; dt.add(5, 'minutes')) {
      ...
      if (options.basal) {
        ...
        console.log("basalvalue: ", basalvalue);
    ...
    if (options.basal) {
      ...
      console.log("Basal line: ", linedata);
      g.append('path')
    ...
    //REVERT this position, I don't think this is issue... 
    ...
    function appendProfileSwitch (context, treatment) {
        console.log("Profile treat: ", treatment);
